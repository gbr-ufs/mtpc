...
\hypersetup{
  ...
  pdfcreator={LaTeX with SBCbookchapter},
...
As RNAs possuem limitações críticas em interpretabilidade, eficiência e consumo de recursos. O "problema da caixa preta" é o principal obstáculo: a opacidade dos pesos internos impede a auditoria de decisões em áreas sensíveis \parencite{tjoaSurveyExplainableArtificial2021}. Modelos atuais falham em cenários de Out-of-Distribution (OOD), onde variações mínimas nos dados causam erros graves. Existe também a lacuna da dependência massiva de dados rotulados e alto custo computacional, o que restringe o treinamento de redes profundas a grandes corporações \parencite{10748289}. No ensino de programação, essas falhas são evidentes. Ferramentas baseadas em redes neurais entregam o código final, mas não detalham o processo dedutivo. Essa falta de transparência omite a lógica subjacente, impedindo que o aluno compreenda a causalidade entre o algoritmo e o resultado. A ausência de explicações estruturadas impossibilita a correção de vícios de raciocínio, transformando o aprendizado em uma reprodução mecânica de padrões estatísticos.
...
\subsubsection{Integração Prática com o Mercado}
O potencial do aprendizado profundo para aprimorar a inteligência e a personalização de aplicativos é destacado por \cite{guo_mobile_2024}. O autor analisa a inserção de redes neurais em cursos de desenvolvimento móvel em Java, implementando funcionalidades como reconhecimento de imagem e fala para atender à demanda industrial. A principal lacuna identificada permanece no despreparo dos alunos para lidar com essas inovações, devido à dificuldade de introduzir conceitos complexos de IA em currículos voltados para a prática.

\subsection{Arquitetura Transformer}

A consolidação dos Grandes Modelos de Linguagem atuais fundamenta-se na introdução da arquitetura Transformer, proposta originalmente por \textcite{vaswani_attention_2017}. Até o surgimento dessa arquitetura, o processamento de sequências (como textos e áudios) dependia majoritariamente de Redes Neurais Recorrentes (RNNs) e unidades \textit{Long Short-Term Memory} (LSTM). Tais modelos processavam os dados de forma sequencial, palavra por palavra, o que impedia a paralelização eficiente do treinamento e limitava a capacidade do sistema de \enquote{lembrar} contextos distantes dentro de um parágrafo longo. O modelo Transformer rompeu com esse paradigma ao dispensar a recorrência em favor de mecanismos de atenção pura, permitindo o processamento simultâneo de toda a sequência de dados.
...
Outro componente crítico introduzido para compensar a ausência de processamento sequencial é a Codificação Posicional (\textit{Positional Encoding}). Segundo \textcite{jurafsky_speech_2024}, como o \textit{Transformer} \enquote{enxerga} a frase inteira de uma vez, ele não possui uma noção inerente de ordem (saber qual palavra vem antes ou depois). Para solucionar isso, injeta-se informações sobre a posição relativa ou absoluta dos \textit{tokens} diretamente nas representações de entrada, permitindo que a rede distinga a ordem sintática dos elementos. Essa estrutura é composta por blocos empilhados de codificadores e decodificadores, embora muitos modelos modernos (como a série GPT) utilizem apenas a pilha de decodificadores.
...
\begin{enumerate}
\item A presente investigação classifica-se como aplicada, de natureza exploratória e descritiva, com uma abordagem mista (qualiquantitativa).
\item Natureza Aplicada: O estudo visa resolver um problema prático identificado no ambiente académico: a necessidade de integrar ferramentas de IA no ensino de programação sem comprometer o desenvolvimento cognitivo dos discentes.
\item Natureza Exploratória e Descritiva: Exploratória devido à recência do fenómeno dos Large Language Models (LLMs) e a escassez de dados específicos sobre o seu impacto na Universidade Federal de Sergipe (UFS); e descritiva, pois procura mapear e descrever as características de uso e as percepções de alunos e professores.
\item Contexto: A pesquisa será realizada no Departamento de Computação da UFS, tendo como público-alvo os discentes ingressantes do período letivo 2025.1 matriculados nas disciplinas introdutórias de Programação Funcional e Programação Imperativa.
\end{enumerate}

\subsection{Perguntas de Pesquisa}
Os objetivos específicos deste trabalho foram desdobrados nas seguintes questões de pesquisa (QP), que guiam a investigação:
\begin{enumerate}
\item QP1: Com que frequência e intensidade os alunos ingressantes utilizam ferramentas de IA Generativa para a resolução de atividades de programação?
\item QP2: Qual é a percepção dos discentes sobre os efeitos dessas ferramentas na capacidade cognitiva, especificamente na memória e raciocínio lógico?
\item QP3: Existe uma correlação direta entre o nível de complexidade dos problemas de programação propostos e a recorrência ao uso de ferramentas generativas pelos estudantes?
\end{enumerate}

\subsection{Materiais e Ferramentas}
Para a operacionalização da pesquisa, foram selecionados os seguintes recursos, com base na sua acessibilidade e capacidade de análise de dados:
\begin{enumerate}
\item Instrumentos de Coleta: Formulários eletrônicos (Google Forms) para a aplicação de questionários estruturados aos discentes.
\item Ferramentas de Análise: Linguagem Python com bibliotecas de análise de dados para o processamento estatístico das respostas quantitativas e identificação de padrões de uso.
\item Bases de Dados Bibliográficas: IEEE Xplore, ACM Digital Library e Google Scholar, utilizadas para a fundamentação teórica e comparação com trabalhos relacionados.
\end{enumerate}

\subsection{Procedimentos Metodológicos}
Os procedimentos adotados dividem-se em quatro fases estratégicas:
\begin{enumerate}
\item Revisão Sistemática da Literatura: Levantamento do estado da arte sobre LLMs na educação, focando em estudos recentes que abordam a "dívida cognitiva" e o "aprendizado superficial".
\item Desenvolvimento do Instrumento: Elaboração de um questionário validado para validar as hipóteses de dependência tecnológica e redução da prática de debugging.
\item Coleta de Dados: Aplicação do questionário aos alunos das disciplinas alvo ao final do semestre letivo, garantindo uma amostragem representativa dos ingressantes.
\item Análise e Triangulação: Processamento dos dados quantitativos para responder às QPs e comparação qualitativa com as percepções recolhidas, validando se o cenário local reflete as tendências globais identificadas no referencial teórico.
\end{enumerate}

\subsection{Etapas da Pesquisa}
O cronograma de execução foi estruturado nas seguintes etapas, com os respectivos entregáveis:
\begin{enumerate}
\item Etapa 1 (Fundamentação): Definição do problema e escrita do Referencial Teórico. Entregável: Capítulo 2 do artigo.
\item Etapa 2 (Instrumentação): Definição das perguntas do questionário e seleção da população-alvo. Entregável: Link do formulário de pesquisa.
\item Etapa 3 (Campo): Coleta de respostas junto aos discentes de Programação Funcional e Programação Imperativa. Entregável: Base de dados bruta.
\item Etapa 4 (Análise): Tratamento dos dados e geração de gráficos de correlação. Entregável: Resultados preliminares.
\item Etapa 5 (Conclusão): Redação final, discussão dos resultados e defesa. Entregável: Artigo completo submetido.
\end{enumerate}

\subsection{Validação e Avaliação}
A eficácia da metodologia será avaliada através da consistência estatística dos dados coletados e da capacidade de responder às perguntas de pesquisa formuladas. A validação ocorrerá pela comparação dos resultados obtidos na UFS com estudos similares, como os realizados pelo MIT (\parencite{kosmynaYourBrainChatGPT2025}) e Microsoft (\parencite{leeImpactGenerativeAI2025}), permitindo verificar se o fenómeno da redução do desempenho em tarefas não assistidas se repete no contexto local.

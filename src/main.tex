...
A presente investigação classifica-se como aplicada, de natureza exploratória e descritiva, com uma abordagem mista (qualiquantitativa).

Natureza Aplicada: O estudo visa resolver um problema prático identificado no ambiente académico: a necessidade de integrar ferramentas de IA no ensino de programação sem comprometer o desenvolvimento cognitivo dos discentes.

Natureza Exploratória e Descritiva: Exploratória devido à recência do fenómeno dos Large Language Models (LLMs) e a escassez de dados específicos sobre o seu impacto na Universidade Federal de Sergipe (UFS); e descritiva, pois procura mapear e descrever as características de uso e as percepções de alunos e professores.

Contexto: A pesquisa será realizada no Departamento de Computação da UFS, tendo como público-alvo os discentes ingressantes do período letivo 2025.1 matriculados nas disciplinas introdutórias de Programação Funcional e Programação Imperativa.
...
Instrumentos de Coleta: Formulários eletrônicos (Google Forms) para a aplicação de questionários estruturados aos discentes.

Ferramentas de Análise: Linguagem Python com bibliotecas de análise de dados para o processamento estatístico das respostas quantitativas e identificação de padrões de uso.

Bases de Dados Bibliográficas: IEEE Xplore, ACM Digital Library e Google Scholar, utilizadas para a fundamentação teórica e comparação com trabalhos relacionados.
